...
export function getAll({ search, category, sort } = {}) {
  ...
  if (category === "recent") {
    sql += " ORDER BY lastAccessedAt DESC";
    if (!search) sql += " LIMIT 20";
  } else {
...
export function findDuplicates(candidates) {
  const all = db.prepare("SELECT name, username, url FROM entries").all();
  const existingKeys = new Set(
    all.map((e) => `${e.name}|${decrypt(e.username) || ""}|${decrypt(e.url) || ""}`)
  );
  return candidates.map((c) => existingKeys.has(`${c.name}|${c.username || ""}|${c.url || ""}`));
}
